...
def get_air_quality(city):
    ...
    try:
        url = 'http://api.waqi.info/feed/{city}/?token={token}'.format(
            city=city, token=AQICN_TOKEN)
            ...
            if content_dict.get('status') == 'ok':
                ...
                for key in sorted(AIR_STATUS_DICT):
                    if key >= aqi:
                        ...
                        break
                aqi_info = '{city} PM2.5：{aqi} {air_status}'.format(
                    city=city, aqi=aqi, air_status=air_status)
                # print(aqi_info)
                return aqi_info
            else:
