...
from app.utils.common import SPIDER_HEADERS
...
def get_xzw_horoscope(name, is_tomorrow=False):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            lucky_color = re.findall(
                r'<label>幸运颜色：</label>(.*?)</li>', html)[0]
            detail_horoscope = re.findall(
                r'<p><strong class="p1">.*?</strong><span>(.*?)</span></p>', html)[0]
            if is_tomorrow:
...
if __name__ == '__main__':
    ...
    is_tomorrow = True
    print(get_xzw_horoscope("天秤座", is_tomorrow))
