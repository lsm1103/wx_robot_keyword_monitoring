...
def do_command(msg):
    ...
    if '开启监控销售群聊回复' == msg.text:
        msg.bot.is_listen_zy_groups = True
        msg.reply('已开启监控销售群群聊回复')
        return None

    if '关闭监控销售群聊回复' == msg.text:
        msg.bot.is_listen_zy_groups = False
        msg.reply('已关闭监控销售群群聊回复')
        return None
    ...
    if '开启监听模式' == msg.text:
        ...
        if errmsg:
            ...
            msg.reply('已开启监听模式，在{0}中监听{1}'.format(
                str(msg.bot.listen_friend_groups), str(msg.bot.listen_friends)))
        return None
